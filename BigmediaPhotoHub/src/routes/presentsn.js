...
router.get('/:side_number', asyncMW(async (req, res) => {
    let {side_number} = req.params
    side_number = parseInt(side_number);
    const data = await PhotosPresent.findOne({ attributes: ['sideId','path','city','address','cathegory','supplier_sn','sizetype','sidetype','side_number'], where : { side_number } })
    if(!data) return;

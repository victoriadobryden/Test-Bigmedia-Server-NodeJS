Ext.define('Bigmedia.view.dialog.DlgSearchPoiController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.dialog-dlgsearchpoi',
    ...
    onGetPOIClick: function () {
        var me = this,
            view = me.getView(),
            poiNameContainer = me.lookup('wizard').lookup('poinamecontainer'),
            poiCats = me.lookup('wizard').lookup('treeCat'),
            sb = me.lookup('searchstatus'),
            poiStore = view.getPoiStore(),
            names = [], cats = [],
            curCamp = view.lookupViewModel().get('curCamp'),
            filters = curCamp.get('filters'),
            cities;
        // return;
        if (filters && filters['x-gridfilter-city']) {
          cities = filters['x-gridfilter-city'].value;
        }
        sb.showBusy();
        poiNameContainer.items.each(function(field){
            if (field.getValue()) {
                names.push(field.getValue());
            }
        });
        poiCats.getChecked().forEach(function(node){
            cats.push(node.get('id'));
        });
        Ext.Ajax.request({
            url: '/api/v1/pois/',
            method: 'POST',
            jsonData: {cats: cats, names: names}
        }).then(function(response, opts) {
            var json = Ext.decode(response.responseText);
            var toAdd = json.filter((rec)=>(!cities || cities.indexOf(rec.city)>=0)).map((rec)=>{
              rec.name = rec['name_' + Bigmedia.Locales.currentLocale] || rec['name_ukr'] || rec['name_ru'] || rec['name_en'] || '';
              rec.geometry = (parseFloat(rec.lon) && parseFloat(rec.lat)) ? new ol.geom.Point(ol.proj.fromLonLat([parseFloat(rec.lon), parseFloat(rec.lat)])) : null;
              rec.centroid = rec.geometry;
              if (poiStore.getAssociatedEntity()) {
                delete rec.id;
                rec[poiStore.foreignKeyName] = poiStore.getAssociatedEntity().getId();
              }
              return rec;
            });
            poiStore.add(toAdd);
    ...
    updateSearchCondition: function () {
        ...
        Ext.Ajax.request({
            url: '/api/v1/pois/count',
            method: 'POST',
            jsonData: {cats: cats, names: names, countOnly: true}
        }).then(function(response, opts) {
            var json = Ext.decode(response.responseText);
            // console.log(json);
            var count = json && json.length ? json[0].poi_count : 0;
            var status = 'Found ' + count + ' POI';
            sb.setStatus({
                text: status,
    ...
    clearTreeView: function () {
        var treeCat = this.lookup('wizard').lookup('treeCat');

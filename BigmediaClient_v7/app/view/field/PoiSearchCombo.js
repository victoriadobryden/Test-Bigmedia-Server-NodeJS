Ext.define("Bigmedia.view.field.PoiSearchCombo", {
    extend: "Ext.form.field.ComboBox",
    xtype: "poisearchcombobox",

    requires: [
        "Bigmedia.Locales",
        'Bigmedia.store.PoiSearch'
    ],
    ...
    setTargetGrid: function (val) {
        this.targetGrid = val;
    },
    setFilterField: function (val) {
        this.filterField = val;
    },
    initTag: function () {
        this.labelTpl = new Ext.XTemplate('{name}<tpl if="subCount &gt; 0"><span class="tagfield-badge">-{subCount}</span></tpl>', { //{[this.inGrid(values.id, targetGrid, filterField)]}
                disableFormats: true
                //definitions: 'var targetGrid = "' + this.targetGrid + '", filterField = "' + this.filterField + '";',
                //inGrid: function (id, targetGrid, filterField) {
                //    //console.log('labelTpl: inGrid ' + targetGrid);
                //    var recs = Ext.getCmp(targetGrid).getStore().getRange();
                //    return recs.reduce(function (count, item) {
                //        if (item.get(filterField) === id) {
                //            count++;
                //        }
                //        return count;
                //    }, 0);
        ...
        //this.tipTpl = new Ext.XTemplate('Shown {addCount} from total {totalCount}',{disableFromats: true}
        this.tipTpl = new Ext.XTemplate(Bigmedia.Locales.refTagTip, {disableFromats: true}
        );
        this.tpl = new Ext.XTemplate(
            '<ul class="x-list-plain"><tpl for=".">',
            '<li role="option" class="x-boundlist-item <tpl if="addCount &lt;= 0">x-boundlist-item-disabled</tpl>">{category}: {name}<tpl if="addCount &gt; 0"><span class="tagfield-badge">+{addCount}</span></tpl></li>', //{[this.inGrid(values.id, targetGrid, filterField)]}
            '</tpl></ul>',
            {
                disableFormats: true
            }
        ...
        change: function (combo, newVal, oldVal) {
            ...
            if (newVal !== oldVal) {
                ...
                if (newVal.length > 0) {
                    ...
                    if (oldVal.length > 0) {
                        combo.suppressRefresh = true;
                        store.removeFilter('filter' + combo.filterField, true);
                    }
                    var refStore = this.getStore();
                    var faces = [];
                    //refStore.each(function (rec) {
                    ...
                    // console.log(faces);
                    var streetFilter = new Ext.util.Filter({
                        property: 'id',
                        id: 'filter' + combo.filterField,
                        operator: 'in',
                        value: faces
                    });
                    //console.log(newVal);
                    //combo.suppressRefresh = true;
                    combo.suppressRefresh = true;
                    Ext.getCmp(combo.targetGrid).getStore().addFilter(streetFilter);
                    //combo.updateValue();
                }
                else {
                    combo.suppressRefresh = true;
                    Ext.getCmp(combo.targetGrid).getStore().removeFilter('filter' + combo.filterField);
                }
            }
        }
    }

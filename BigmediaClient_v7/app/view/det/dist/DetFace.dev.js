...
Ext.define('Bigmedia.view.det.DetFace', {
  ...
  requires: ['Bigmedia.view.drag.Image', 'Ext.form.Panel', 'Ext.form.field.Text', 'Ext.form.field.File', 'Bigmedia.view.det.DetSchema'],
  xtype: 'detface',
  title: '',
  width: 300,
  height: 250,
  ...
  updateRecord: function updateRecord(newVal) {
    var me = this;
    me.setFace(newVal);
  },
  updateFace: function updateFace(newVal) {
    var me = this,
    ...
    if (newVal) {
      if (newVal.get('doorsNo') != null) {
        doors.setTitle('DoorsNo. ' + newVal.get('doorsNo')); // doors.setHtml('<div style="background: "></div>')
      ...
      me.setTitle(title); // console.log(`newVal.get('photos') ->${newVal.get('photos')}, newVal.get('photoUrl')-> ${newVal.get('photoUrl')}`);
      ...
      if (newVal.get('photos') || newVal.get('photoUrl')) {
        ...
        var facesInfoStore = Ext.getStore('FacesInfo'),
            ...
            faceId = newVal.get('faceId'); // console.log('newVal', newVal );
        // console.log('sideId', sideId );

        var info = facesInfoStore.getById(photoId); // var info = facesInfoStore.getById(faceId);
        // console.log('info', info );

        if (!info) {
          Bigmedia.model.FaceInfo.load(photoId, {
            // Bigmedia.model.FaceInfo.load(faceId, {
            success: function success(record, operation) {
              // console.log(record);
  ...
  updateMap: function updateMap(newVal) {// var me = this;
    // if (newVal) {
  ...
  tools: [{
    ...
    handler: function handler(btn, pressed) {
      var me = this,
          win = me.up('window'),
          detschema = win.up().lookup('detschema'); // detschema = win.lookup('detschema');
    ...
    // xtype: 'button',
    bind: {
      disabled: '{!record.doorsNo}' // pressed: '{!doorsphoto.hidden}'

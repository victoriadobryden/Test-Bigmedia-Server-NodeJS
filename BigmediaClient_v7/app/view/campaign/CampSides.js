Ext.define('Bigmedia.view.campaign.CampSides',{
    ...
    requires: [
        ...
        'Bigmedia.view.dialog.DlgImportSupplierPrices',
        'Bigmedia.view.dialog.DlgImportPrices'
    ],

    controller: 'campaign-campsides',
            ...
            items:
            {
                ...
                dockedItems: [
                    ...
                    },
                    {
                        ...
                        text: Bigmedia.Locales.statusBarLoading,
                ...
                listeners: {
                    beforeedit: function (editor, e) {
                        var me = this;
                        if (e.field === 'netPrice' && (!Bigmedia.Vars.getSalt(e.record.get('campaignId')) || (e.record.get('supplierPriceEnc') && !e.value))) {
                            Bigmedia.Vars.showPriceEncryptDialog(e.record.get('campaignId'), function() {
                                (e.grid.getStore().getData().getSource() || e.grid.getStore().getData()).each(function(rec){
                                    rec.set('netPrice', rec.get('netPrice'));
                                });
                            });
                            return false;
                    ...
                    edit: function (editor, e) {
                        ...
                        if (e.field === 'netPrice') {
                            ...
                            e.record.set('supplierPriceEnc', priceEnc);
                            // e.record.commit();
                        } else if (e.field === 'displayPubPrice') {
                            var pubCamp = me.lookupViewModel().get('pubCamp'),
                                savedPassphrase = Bigmedia.Vars.getPubPassphrase(e.record.get('pubCampaignId'));
    ...
    updateStatusBar: function () {
        console.log('here updateStatusBar');
        var me = this,
        ...
        var ots = [store.min('ots') || 0, store.max('ots'), Math.round(store.average('ots') * 100) / 100, Math.round(store.sum('ots') * 100) / 100];

        status += '  | OTS(min,max,avg) = ' + ots.slice(0,3).join(', ') + ' | OTS(sum) = ' + ots[3];

        var cities = store.collect('city');
        if(cities.length == 1) {
            grp = [store.min('grp') || 0, store.max('grp'), Math.round(store.average('grp') * 100) / 100, Math.round(store.sum('grp') * 100) / 100];
